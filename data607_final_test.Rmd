...
l.out <- BatchGetSymbols(tickers = "ABNB",
                         first.date = first.date,
                        last.date = last.date, do.cache=FALSE)

print(l.out$df.control)
print(l.out$df.tickers)
out_test_3 <- l.out$df.tickers$price.high
...
t <- ggplot(sample_data) + aes(x=date_utc, y=sentiment) + geom_boxplot() + stat_summary(fun = "median",
              ...
              stat_summary(fun = "mean",
               ...
               colour = "red")
plot1 <- ggplotly(t)
fig <- subplot(plot1, plot1, nrows = 2, shareX = TRUE) %>%  layout(xaxis = list(showspikes = TRUE, spikemode  = "toaxis+across+marker",
spikesnap = "cursor",
showline=TRUE,
showgrid=TRUE,
